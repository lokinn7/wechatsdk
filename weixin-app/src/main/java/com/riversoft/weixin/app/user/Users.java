...
public class Users {
	...
	public static Users defaultUsers(String appId, String secret) {
		return with(new AppSetting(appId, secret));
	}
	...
	public SessionKey code2Session(String code) {
		...
		String sessionKey = wxClient.get(String.format(url,
				wxClient.getClientId(), wxClient.getClientSecret(), code));
		logger.debug("code to session key: {}", sessionKey);
		return JsonMapper.nonEmptyMapper().fromJson(sessionKey,
				SessionKey.class);
	}
}
