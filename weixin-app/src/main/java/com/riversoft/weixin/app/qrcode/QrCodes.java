...
public class QrCodes {
     ...
     * @return
     */
    public InputStream create(String path, String scene, int size, boolean ishyaline) {
        String url = WxEndpoint.get("url.qrcode.createunlimit");
        Map<String, Object> params = new HashMap<>();
        params.put("path", path);
        params.put("scene", scene);
        params.put("width", size);
        params.put("is_hyaline", ishyaline);
        String json = JsonMapper.defaultMapper().toJson(params);
        return wxClient.copyStream(url, String.format(json, path, scene, ishyaline, size));
    }
}
